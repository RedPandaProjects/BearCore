...
void BearCore::BearBufferedReader::ReadStringToLine(BearString &str, BearEncoding::Encoding encoding) const
{
...
#elif UTF_8
		str.append(*BearEncoding::ToUTF8(Begin1, Begin2), Begin2 - Begin1);
#else
	...
	{
		const bchar_utf8 *Begin1 = reinterpret_cast<const bchar_utf8 *>(Begin());
		const bchar_utf8 *End1 = reinterpret_cast<const bchar_utf8 *>(End());
		const bchar_utf8 *Begin2 = reinterpret_cast<const bchar_utf8 *>(strchr(reinterpret_cast<const bchar8 *>(Begin1), '\r'));
		if (!Begin2)
			Begin2 = reinterpret_cast<const bchar_utf8 *>(strchr(reinterpret_cast<const bchar8 *>(Begin1), '\n'));
		if (!Begin2 || Begin2 > End1)
		{
			Begin2 = End1;
		}
...
void BearCore::BearBufferedReader::ReadStringToZero(BearString &str, BearEncoding::Encoding encoding) const
{
	...
	case BearEncoding::UTF8:
	{
		const bchar_utf8 *Begin1 = reinterpret_cast<const bchar_utf8 *>(Begin());
		const bchar_utf8 *End1 = reinterpret_cast<const bchar_utf8 *>(End());
		const bchar_utf8 *Begin2 = reinterpret_cast<const bchar_utf8 *>(strchr(reinterpret_cast<const bchar8 *>(Begin1), '\0'));
		if (!Begin2)
		{
			Begin2 = End1;
		}
...
#elif UTF_8
		str.append(*BearEncoding::ToUTF8(Begin1, Begin2));
#else
