...
{
	template<class C>
	class BearVector :public std::vector<C,bear_allocator<C>>
	{
	private:
		typedef	std::vector<C, bear_allocator<C>> vector;
	public:
		...
		BearPtr<C> popData() { auto ptr = BearPtr<C>(&at(0), true); this->_Myfirst() = vector::pointer(); this->_Mylast() = vector::pointer(); this->_Myend() = vector::pointer(); return ptr; }
