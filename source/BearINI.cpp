...
void BearINI::decoder(const BearBufferedReader & base, BearEncoding::Encoding typetext, BearIncluder*includer)
{
	...
	while (!base.Eof())
	{
		...
		if (str.exist_possition(0, TEXT("#include")))
		{
			str.seek(str.tell() + 9);
			str.sub_space_in_begin();
			BEAR_FATALERROR(**str==TEXT("\"")[0], TEXT("Неожиданный конец [" BEAR_PRINT_STR_CURRENT "]"), **str);
			str.sub_space_in_end();
			BEAR_FATALERROR((*str)[str.size()-1] == TEXT("\"")[0], TEXT("Неожиданный конец [" BEAR_PRINT_STR_CURRENT "]"), **str);
			(*str)[str.size() - 1] = 0;
		...
		if (str[0] == TEXT("[")[0])
		{
			...
			str++;
			bchar*data = *str;
			BEAR_FATALERROR(str.to_char(TEXT("]")[0]), TEXT("Нет конца в название секции [" BEAR_PRINT_STR_CURRENT "]"), data);
			str.sub_space_in_begin();
...
const BearVector<BearString>& BearINI::Key(BearStringConteniar section, BearStringConteniar key) const
{
	...
	BEAR_FATALERROR(sec != sections.end(), TEXT("Секция [" BEAR_PRINT_STR_CURRENT "] не существует "), *section);
	auto k = sec->second.keys.find(key);
	BEAR_FATALERROR(k != sec->second.keys.end(), TEXT("Ключь в секции [" BEAR_PRINT_STR_CURRENT "] не сущесвтует [" BEAR_PRINT_STR_CURRENT "]"), *key, *section);
	return 	k->second;
}
