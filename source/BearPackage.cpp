...
BearCore::BearPackage::BearPackage()
{
}
BearCore::BearPackage::~BearPackage()
{
}
void BearCore::BearPackage::Compressor(const bchar * file_out, const bchar*path_to_files, const bchar*pathfs_for_archive, const bchar*path_for_archive, BearVector<BearString>& file_list)
{
	...
	for (bsize i = 0; i < file_list.size(); i++)
	{
		...
		while (!Fin.Eof())
		{
			Fout.Write(buffer, Fin.Read(buffer, 64 * 1024));
		}
	}
}

bool BearCore::BearPackage::LoadFromFile(const bchar * file)
{
...
bool BearCore::BearPackage::ExistFile(const bchar * file)
{
	BearStringPath path;
...
BearCore::BearFilePackageStream BearCore::BearPackage::FileOpen(const bchar * file)
{
	BearStringPath path;
	BearString::Copy(path, file);
	BearFileManager::PathOptimization(path);
	auto item = m_files.find(BearStringConteniar(file, false));
	if (item != m_files.end())
	{
...
void BearCore::BearPackage::Clear()
{
	m_file.clear();
	m_files.clear();
}

BearCore::BearPackage::BearPackage(const BearPackage & package)
{
	Copy(package);
}

BearCore::BearPackage::BearPackage(BearPackage && package)
{
	Swap(package);
}

void BearCore::BearPackage::Swap(BearPackage & package)
{
...
void BearCore::BearPackage::Copy(const BearPackage & package)
{
...
BearCore::BearPackage & BearCore::BearPackage::operator=(const BearPackage & package)
{
	Copy(package);
	return*this;
...
BearCore::BearPackage & BearCore::BearPackage::operator=(BearPackage && package)
{
	Swap(package);
	return*this;
	// TODO: вставьте здесь оператор return
