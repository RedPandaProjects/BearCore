...
void BearCore::BearInputStream::readString(BearString & str, bsize size, BearEncoding::Encoding type) const
{
	...
	{
#ifdef UNICODE
		auto te = BearEncoding::ToUTF16(reinterpret_cast<const bchar*>(*text), reinterpret_cast<const bchar*>(*text+size));
		str.append(*te);
#else 
...
#else 
		auto te = BearEncoding::ToANSI(reinterpret_cast<const bchar*>(*text), reinterpret_cast<const bchar*>(*text + size));
		str.append(*te);
#endif
	}
	return;
