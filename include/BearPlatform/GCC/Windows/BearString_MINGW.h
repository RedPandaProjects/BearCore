...
inline bsize BearString::Printf(bchar_utf8 (&dst)[sizeBuffer], const bchar_utf8 *str, ...)
{
	va_list va;
	va_start(va, str);
	int result = vsprintf((char *)dst, (const char *)str, va);
	va_end(va);
	return static_cast<bsize>(result);
}
template <bsize sizeBuffer>
...
inline void BearString::ToLower(bchar8 *p)
{
	while (*p)
	{
...
inline void BearString::ToUpper(bchar8 *p)
{
	while (*p)
	{
...
inline void BearString::ToLower(bchar16 *p)
{
	while (*p)
	{
		*p = towlower(*p);
...
inline void BearString::ToUpper(bchar16 *p)
{
	while (*p)
	{
		*p = towupper(*p);
...
inline int32 BearString::CompareWithoutCase(const bchar8 *a, const bchar8 *b)
{
	int ca, cb;
	do
	{
...
inline int32 BearString::CompareWithoutCase(const bchar16 *a, const bchar16 *b)
{
	wint_t ca, cb;
	do
	{
